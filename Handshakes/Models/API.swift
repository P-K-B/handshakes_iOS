...
//                    var j=0
                    for shake_path in statusResponce.payload.path!{
                        ...
                        var decodedPath = SearchPathDecoded(path_id: shake_path.key, depth: path_len, path: [[]])
                        if (path_len == 2){
                            decodedPath = decodedPathes.first(where: {$0.depth == path_len}) ?? SearchPathDecoded(path_id: shake_path.key, depth: path_len, path: [[]])
                            var i=0;
...
//                                        lines += count.count
                            ...
                            decodedPath = SearchPathDecoded(path_id: shake_path.key, depth: path_len, path: [[]])
                            var i=0;
                            for row in shake_path.value{
                                if (i != 0){
                                    if (!(decodedPath.path[0].contains(row.phone ?? ""))){
